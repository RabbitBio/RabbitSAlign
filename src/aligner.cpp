...
AlignmentInfo Aligner::align_gpu(const std::string &query, const std::string &ref, gasal_tmp_res &gasal_res) const {
    m_align_calls++;
    AlignmentInfo aln;
    ...
    auto flag = ssw_aligner.Align_gpu(gasal_res, query.c_str(), ref.c_str(), ref.size(), filter, &alignment_ssw, maskLen);
    if (flag != 0) {
        aln.edit_distance = 100000;
        aln.ref_start = 0;
        aln.sw_score = -100000;
...
AlignmentInfo Aligner::align_gpu(const std::string_view &query, const std::string_view &ref, gasal_tmp_res &gasal_res) const {
    m_align_calls++;
    AlignmentInfo aln;
    int32_t maskLen = query.length() / 2;
    maskLen = std::max(maskLen, 15);
    ...
    // query must be NULL-terminated
    auto flag = ssw_aligner.Align_gpu(gasal_res, query.data(), query.length(), ref.data(), ref.length(), filter, &alignment_ssw, maskLen);
    if (flag != 0) {
        aln.edit_distance = 100000;
        aln.ref_start = 0;
        aln.sw_score = -100000;
...
AlignmentInfo Aligner::align(const std::string_view &query, const std::string_view &ref) const {
    m_align_calls++;
    AlignmentInfo aln;
    int32_t maskLen = query.length() / 2;
    maskLen = std::max(maskLen, 15);
    ...
    // query must be NULL-terminated
    auto flag = ssw_aligner.Align(query.data(), query.length(), ref.data(), ref.length(), filter, &alignment_ssw, maskLen);
    if (flag != 0) {
        aln.edit_distance = 100000;
        aln.ref_start = 0;
        aln.sw_score = -100000;
