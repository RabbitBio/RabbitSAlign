...
int set_up_bb_segsort(int *&d_bin_segs_id, int *&d_bin_counter, int length) {
...
int free_bb_segsort(int *d_bin_segs_id, int *d_bin_counter) {
    cudaError_t cuda_err;
...
int bb_segsort(K *keys_d, T *vals_d, K *keysB_d, T *valsB_d, int n,  int *d_segs, int *d_bin_segs_id, int *d_bin_counter, int length)
{   
    ...
    blocks.x = 256;                                     //threads per block
    subwarp_size = 2;                                   //threads for one segment
    subwarp_num = h_bin_counter[2]-h_bin_counter[1];    //num of segments
    ...
    if(subwarp_num > 0)
    my_t2_ppt1_orig<<<grids, blocks, 0, streams[1]>>>(keys_d, vals_d, keysB_d, valsB_d,
    ...
    if(subwarp_num > 0)
    my_t2_ppt2_orig<<<grids, blocks, 0, streams[2]>>>(keys_d, vals_d, keysB_d, valsB_d,
    ...
    grids.x = (subwarp_num+factor-1)/factor;
    if(subwarp_num > 0)
    my_t2_ppt4_orig<<<grids, blocks, 0, streams[3]>>>(keys_d, vals_d, keysB_d, valsB_d,
    ...
    grids.x = (subwarp_num+factor-1)/factor;
    if(subwarp_num > 0)
    my_t4_ppt4_strd<<<grids, blocks, 0, streams[4]>>>(keys_d, vals_d, keysB_d, valsB_d,
    ...
    grids.x = (subwarp_num+factor-1)/factor;
    if(subwarp_num > 0)
    my_t8_ppt4_strd<<<grids, blocks, 0, streams[5]>>>(keys_d, vals_d, keysB_d, valsB_d,
    ...
    grids.x = (subwarp_num+factor-1)/factor;
    if(subwarp_num > 0)
    my_t16_ppt4_strd<<<grids, blocks, 0, streams[6]>>>(keys_d, vals_d, keysB_d, valsB_d, 
    ...
    grids.x = (subwarp_num+factor-1)/factor;
    if(subwarp_num > 0)
    my_t8_ppt16_strd<<<grids, blocks, 0, streams[7]>>>(keys_d, vals_d, keysB_d, valsB_d,  
    ...
    grids.x = (subwarp_num+factor-1)/factor;
    if(subwarp_num > 0)
    my_t32_ppt8_strd<<<grids, blocks, 0, streams[8]>>>(keys_d, vals_d, keysB_d, valsB_d,  
