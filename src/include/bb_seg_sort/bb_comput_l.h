...
__global__ 
void calc_segment_info(int *segs, int *bin, int bin_size, int *starts, int *ends) 
{
    const int gid = threadIdx.x + blockIdx.x * blockDim.x;
    
    if (gid < bin_size) {
        int segs_offset = bin[gid];
        int start = segs[segs_offset];
        int end = segs[segs_offset+1];
...
template<class K, class T>
int sort_cub_r65537(K *keys_d, T *vals_d, K *keysB_d, T *valsB_d, 
        int n, int *segs_d, int *bin_d, int bin_size, int length, cudaStream_t stream)
{
    cudaError_t err;

    int *d_seg_starts = nullptr;
    int *d_seg_ends = nullptr;
    err = cudaMalloc(&d_seg_starts, sizeof(int) * bin_size);
    ERR_INFO(err, "malloc d_seg_starts");
    err = cudaMalloc(&d_seg_ends, sizeof(int) * bin_size);
    ERR_INFO(err, "malloc d_seg_ends");

    dim3 blocks(256, 1, 1);
    dim3 grids((bin_size+blocks.x-1)/blocks.x, 1, 1);

    calc_segment_info<<<grids, blocks, 0, stream>>>(segs_d, bin_d, bin_size, d_seg_starts, d_seg_ends);

    // std::cout << "seg_starts:\n"; for(auto k: h_seg_starts) std::cout << k << ", "; std::cout << std::endl;
    // std::cout << "seg_ends:\n"; for(auto k: h_seg_ends) std::cout << k << ", "; std::cout << std::endl;

    void *d_temp_storage = NULL;
    size_t temp_storage_bytes = 0;

    cub::DeviceSegmentedRadixSort::SortPairs(d_temp_storage, temp_storage_bytes, 
        keys_d, keysB_d, vals_d, valsB_d, n, bin_size, d_seg_starts, d_seg_ends,
        0, sizeof(K) * 8, stream, false);
    ...
    ERR_INFO(err, "free d_temp_storage");

    err = cudaFree(d_seg_starts);
    ERR_INFO(err, "free d_seg_starts");
    err = cudaFree(d_seg_ends);
    ERR_INFO(err, "free d_seg_ends");
