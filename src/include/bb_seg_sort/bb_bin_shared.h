...
#ifndef _H_BB_BIN_SHARED
#define _H_BB_BIN_SHARED

#include <thrust/device_ptr.h>
#include <thrust/scan.h>

#define SEGBIN_NUM_SHARED 12

__global__
void bb_bin_histo_shared(int *d_bin_counter, const int *d_segs, int length, int n);

__global__
void bb_bin_group_shared(int *d_bin_segs_id, int *d_bin_counter, const int *d_segs, int length, int n);

void bb_bin_shared(int *d_bin_segs_id, int *d_bin_counter, const int *d_segs, 
        const int length, const int n, int *h_bin_counter)
{
    const int num_threads = 256;
    const int num_blocks = ceil((double)length/(double)num_threads);

    bb_bin_histo_shared<<< num_blocks, num_threads >>>(d_bin_counter, d_segs, length, n);

    // show_d(d_bin_counter, SEGBIN_NUM_SHARED, "d_bin_counter:\n");

    thrust::device_ptr<int> d_arr = thrust::device_pointer_cast<int>(d_bin_counter);
    thrust::exclusive_scan(d_arr, d_arr + SEGBIN_NUM_SHARED, d_arr);

    // show_d(d_bin_counter, SEGBIN_NUM_SHARED, "d_bin_counter:\n");

    cudaMemcpyAsync(h_bin_counter, d_bin_counter, SEGBIN_NUM_SHARED*sizeof(int), cudaMemcpyDeviceToHost);

    // group segment IDs (that belong to the same bin) together
    bb_bin_group_shared<<< num_blocks, num_threads >>>(d_bin_segs_id, d_bin_counter, d_segs, length, n);
...
void bb_bin_histo_shared(int *d_bin_counter, const int *d_segs, int length, int n)
{
    ...
    __shared__ int local_histo[SEGBIN_NUM_SHARED];
    if (tid < SEGBIN_NUM_SHARED)
        local_histo[tid] = 0;
    __syncthreads();
